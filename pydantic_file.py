...
def test_users_get_response():
    response = [
        {"id": 153215, "first_name": "Kay", "last_name": "Stivenson"},
        {"id": 484151, "first_name": "Brams", "last_name": "Smit"}
    ]
...
def test_users_get_one_user():
    response = [{"id": 153215, "first_name": "Kay", "last_name": "Stivenson"}]
    users = [User(**user) for user in response]
    assert len(users) == 1
    assert users[0].id == 153215
    assert users[0].first_name == "Kay"
